...
export class StorageBenchmark {
    ...
    generateTestData(messageCount: number, avgCharPerMessage: number): any[] {
        ...
        for (let i = 0; i < messageCount; i++) {
            messages.push({
                ...
                timestamp: new Date(1747258100000),
                origin: 'IDE',
    ...
    async runBenchmarks() {
        ...
        for (const size of testSizes) {
            ...
            for (let i = 0; i < 1; i++) {
                this.lokijsPath = path.join(this.#dbDirectory, `chat-history-test-${i + 8}.json`)
                this.lokijsDbName = `chat-history-test-${i + 8}.json`
                ...
                for (let j = 0; j < 10; j++) {
                    ...
                    for (let k = 0; k < 10; k++) {
                        ...
                        conversations.push({
                            ...
                            messages: testData,
                            clientType: 'vscode',
                            updatedAt: new Date(1747258100000),
                        })
                    }
